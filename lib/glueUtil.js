...
var getLocal = function(options, cb) {

    var streamName = options.inName;
    var localPath = options.localPath;    

    if (streamName.match(/http/g)) { // remote

        var oUrl = url.parse(streamName);
        var sBaseName = path.join(localPath,path.basename(oUrl.pathname));
...
var getStreamFile = function(options,cb) {

    var streamName = options.inName;
    var localPath = options.localPath;

    getLocal(options, function(err, name, urlPath) {
...
var launchGlue = function(urlPath, localName, audioName, oname, cb) {
    ...
    async.eachLimit(list, 100, getFile, function(err) { 
        if (err) {
            console.log(err); 
            cb(err); 
        }
        else {
            var outputFile = outName.split('.')[0] + '.mp4';
            var commandLine = 'ffmpeg -i ' + ffmpegArg + ' -c copy -threads 0 -y ' + outputFile;
